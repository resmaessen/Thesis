...
phase_long = [2*np.pi*(dt/tau)*i for i in range(samples*2)]
K1_full_long = np.asarray([np.sin(phase_long[i]) * K1max for i in range(samples*2)])

## Setting up initial position and reference positions
x1_init = - L/2 #
x2_init = L/2
...
## Remember last positions and create initial dataset
x1_last = x1_init
...
for i in range(1,samples):
    ...
    mu_s_ = mu_s + random.uniform(0, mu_s_random)
    mu_k_ = mu_k + random.uniform(0, mu_k_random)
...
plt.savefig('images/Skill_transfer_chain_1.png')
plt.show()
...
plt.xlabel('time [s]')
plt.legend()
plt.savefig('images/Skill_transfer_phase.png')
plt.show()
