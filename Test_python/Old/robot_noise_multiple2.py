...
def calculate_phase(data_human,DOF, N_AFS, ni, K):
    ...
    return data_human_b, phase_human_b
    
    
def encode_data(data_human, phase_human_b, DOF, N, alpha, beta, lambd, dt, tau):
...
phase_big = np.zeros([trials,samples,DOF])
data_big = np.zeros([trials,samples,DOF])


for i in range(trials):
    if i == 0:
        _ , phase_big[i,:,:] = calculate_phase(data_human,DOF, N_AFS, ni, K)
        data_big[i,:,:] = encode_data(data_human, phase_big[i,:,:], DOF, N, alpha, beta, lambd, dt, tau)
    else:
        _ , phase_big[i,:,:] = calculate_phase(data_big[i-1,:,:],DOF, N_AFS, ni, K)
        data_big[i,:,:] = encode_data(data_big[i-1,:,:], phase_big[i,:,:], DOF, N, alpha, beta, lambd, dt, tau)
...
plt.figure()
# input
plt.plot(time_,data_human[:,1],'r')

for i in range(1):
    plt.plot(time_, data_big[i,:,2])
