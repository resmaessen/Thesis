...
def run_file(runs_ = 12, e_th = p.e_th, close = True):
    ...
    for run in runs:
        ...
        y, dy, ddy = np.array([data_2[0,0]]), np.array([data_2[0,1]]), np.array([0])
        k, dk , ddk = np.array([0]), np.array([0]), np.array([0])
        
        DMP_traject.set_phase( np.array([0]) )
        DMP_traject.set_period( np.array([tau]) )
        
        DMP_stiff.set_phase( np.array([0]) )
        DMP_stiff.set_period( np.array([tau]) )
        
        DMP_traject.learn(y, dy, ddy)
        DMP_stiff.learn(k, dk, ddk)
        ...
        for i in range(1, samples):
            ...
            ddy = K1_real[i]/m * (x - y_old) - (c1/m)*dy_old + k/m * (x - y_old) - (c2/m)*dy_old
