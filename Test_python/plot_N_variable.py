...
fig1, ax1 = plt.subplots(2,len(N), figsize=(20.0, 10.0), sharex=True)
...
for i, N_ in enumerate(N):
    ...
    learning_human_robot.run_file(N=N_, v_N = True)
    learning_robot_robot.run_file(runs, N=N_, v_N = True)
    ...
    ax2[0].plot(t, data_x[0,:,0], label = str(N_))
    ax2[0].set_xticklabels([])
    ax2[0].set_ylabel('x0', fontsize = '13')
        
    for idx in range(1,len(data_x_files)):
        ax2[idx].plot(t, data_x[idx,:,0],label = r"N = " + str(N_) + ' t = ' + str(time_total))
    ...
    ax3[0].plot(t, data_k[0,:,0], label = str(N_))
    ax3[0].set_xticklabels([])
    ax3[0].set_ylabel('K1', fontsize='13')
    
    
    for idx in range(1,len(data_k_files)):
        ax3[idx].plot(t, data_k[idx,:,0], label = r"N = " + str(N_) + ' t = ' + str(time_total))
...
fig1.savefig('images/N/Skill_transfer_N_total.png')
fig2.savefig('images/N/Skill_transfer_N_total_x.png')
fig3.savefig('images/N/Skill_transfer_N_total_k.png')
